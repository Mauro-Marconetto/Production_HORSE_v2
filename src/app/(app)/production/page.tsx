                    ...
                    <div className="flex-grow p-6 grid grid-cols-2 gap-8">
                        <div className="flex flex-col gap-4">
                            {declarationFields.map(({key, label}) => (
                                ...
                                >
                                    <span>{label}</span>
                                    <span className="font-bold text-xl">{prodQuantities[key].toLocaleString()}</span>
                                </Button>
                            ))}
                             <div className="h-20 text-lg justify-between flex items-center px-4 py-2 text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 border border-input bg-background hover:bg-accent hover:text-accent-foreground rounded-md opacity-50 cursor-not-allowed">
                                <span>Piezas Segregadas</span>
                                <span className="font-bold text-xl">{previousSegregatedQty.toLocaleString()}</span>
                             </div>
                        ...
                        <div className="grid grid-cols-3 gap-2">
                             {['1', '2', '3', '4', '5', '6', '7', '8', '9'].map(n => (
                                <Button key={n} variant="outline" className="h-full text-2xl font-bold" onClick={() => handleProdNumericButton(n)}>{n}</Button>
                            ))}
                            <Button variant="outline" className="h-full text-2xl font-bold" onClick={handleProdClear}>C</Button>
                            <Button variant="outline" className="h-full text-2xl font-bold" onClick={() => handleProdNumericButton('0')}>0</Button>
                            <Button variant="outline" className="h-full text-2xl font-bold" onClick={handleProdBackspace}>←</Button>
                        </div>
                    </div>
                )}
